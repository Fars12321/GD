...
func _update_enemies() -> void:
	var day := 1
	if _day_night != null and "current_day" in _day_night:
		day = int(_day_night.current_day)
	var tier := maxi(0, day - 1)
	for enemy in get_tree().get_nodes_in_group("enemy"):
...
func _configure_enemy(enemy: Node, tier: int) -> void:
	if enemy.get_meta("phase41_scaled", false):
		return
	if "max_health" in enemy:
		enemy.max_health = int(round(base_enemy_health * (1.0 + tier * 0.16)))
		enemy.current_health = enemy.max_health
	if "move_speed" in enemy:
		enemy.move_speed = minf(3.7, base_enemy_speed * (1.0 + tier * 0.028))
	if "attack_damage" in enemy:
		enemy.attack_damage = int(round(base_enemy_damage * (1.0 + tier * 0.10)))
