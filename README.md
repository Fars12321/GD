...
> **الإصدار المستهدف: 4.7.2** — يُثبَته CI في كل تشغيل: خطوة `Verify the installed Godot version` تفشل إن كان المُثبَّت غير `4.7.2.*`.
> منذ Godot 4.6 أصبح **Jolt** محرّك الفيزياء الافتراضي، والمشروع لا يثبّت محرّكًا بديلًا؛ لذلك يتحقق `ci/godot_smoke_roguelite.gd` أن الجاذبية والتصادم يعملان فعلًا على المحرّك الجاري (الملك يستقر على الأرض)، لا أن المنطق سليم فقط.
